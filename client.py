...
if __name__ == "__main__":
    """
    Summary: Using the information from repositories.json the following code get the commit information from the 
    repository and add them in the MD files for Git and Json file for HG, for the moment

    Github:
        * call create_git_link to generate the API link for git repos using the repository_name and repository_team
        * call get_commits to generate a list with commits
        * call filter_commit_data to gather only the dates we need from commit
        * call write_commits to add the dates into the general json file
        * create the md file for each repository
        Variables: - repository_name = The name of the repository
                   - repository_team = The name of the team from which the project belongs

    Mercurial:
        * call get_hg_changes to pull the commit information
        * write all the dates into a json file for each repository
        Variables: - repository_url = The URL of repository
                   - repository_push_type = would probably be "json-log" most of the time. We use it to generate the API link
                   - repository_name = contain the name of the repository

    This is a version of using a single file with all the repositories.  
    Maybe we want to add this part in function instead in the MAIN part of the client.py.
    """
